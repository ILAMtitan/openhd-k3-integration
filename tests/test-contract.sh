...
# The installer must reconstruct the known Native-R1 tree from the pinned
# upstream source. The resulting Git tree is the source-identity invariant;
# individual patch-file hashes would only duplicate Git's own integrity model.
...
# Keep the application policy that defines the qualified J722S camera path.
