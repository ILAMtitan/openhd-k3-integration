...
WORK=${WORK:-/var/tmp/openhd-k3-imx415-r0}
BUILD=${BUILD:-/var/tmp/openhd-k3-imx415-build}
...
say 'IMX415 accelerator/runtime preflight'
...
install -d -m 0700 /root/openhd-imx415-checkpoint-backups
[[ -x /usr/local/bin/openhd ]] && cp -a /usr/local/bin/openhd "/root/openhd-imx415-checkpoint-backups/openhd.$STAMP"
[[ -e "$CFG" ]] && cp -a "$CFG" "/root/openhd-imx415-checkpoint-backups/air_camera_generic.$STAMP.json"
[[ -e "$CAMCFG" ]] && cp -a "$CAMCFG" "/root/openhd-imx415-checkpoint-backups/TI_J722S_IMX415_0.$STAMP.json"
[[ -e "$DROPIN" ]] && cp -a "$DROPIN" "/root/openhd-imx415-checkpoint-backups/20-ti-k3-consumer.$STAMP.conf"
...
# Reconstruct and verify the pre-IMX708 qualified R3 tree first.
...
# Add the physically qualified IMX708 checkpoint, then this IMX415 R0 layer.
for p in "$root"/patches/openhd/0009-*.patch "$root"/patches/openhd/0010-*.patch; do
  git -C "$WORK" apply --index --check "$p"
  git -C "$WORK" apply --index "$p"
done

say 'Building OpenHD IMX415 R0'
...
say 'Installing IMX415 OpenHD R0'
...
printf '\nIMX415 R0 installed but OpenHD remains stopped/disabled.\n'
